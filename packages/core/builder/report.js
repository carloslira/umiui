...
const checkVariableWarnings = (variables, filename) => {
  const twVariables = variables.filter(
    (variable) =>
      variable.startsWith("--container") && variable !== "--tw-content"
  );

  const spacingVariables = variables.filter((variable) =>
  ...
  if (
    ...
    console.log(
      `Warning: unnecessary --tw variables in ${filename}:`,
      twVariables
    );
  }
  if (
    spacingVariables.length > 0 &&
    !["spacing.css", "grid.css"].includes(filename)
  ) {
    console.log(
      `Warning: dangerous var(--spacing) variables in ${filename}:`,
      spacingVariables
    );
  }
};
...
const processFile = async (filePath) => {
  ...
  return {
    file: filePath,
    selector: (fileContent.match(/(?:[^}]+{|@\w+\s*[^;{}]+(?:;|\{))/g) || [])
      .length,
...
const report = async (directories) => {
  try {
    ...
    const shouldSave =
      jsonFiles.length === 0 ||
      (await shouldSaveNewReport(
        path.join(reportsDir, jsonFiles[0]),
        flatReport
      ));
