...
const generateColorRules = async ({
  distDir,
  properties,
  breakpoints,
  states,
  ...
  try {
    ...
    const generateBaseVariants = (style, color) =>
      `.${style}-${color}{@apply ${style}-${color};}`;

    const generateOpacityVariants = (style, color, opacityList) =>
      opacityList.map(
        (opacity) =>
          `.${style}-${color}\\/${opacity}{${getStyleProperty(
            style
          )}:color-mix(in oklab,var(--color-${color})${opacity}%,#0000);}`
      );

    const generateResponsiveVariants = (
      style,
      color,
      includeOpacities = []
    ) => {
      const baseVariants = breakpoints.map((bp) =>
        bp.match(/^\d/)
          ? `.\\3${bp[0]}${bp.slice(
              1
            )}\\:${style}-${color}{@apply ${bp}:${style}-${color};}`
    ...
    const compileAndWriteFile = async (content, fileName) => {
      ...
      let extractedContent = compiledContent
    ...
    await Promise.all(
      [
        ...
        outputFiles.states &&
          compileAndWriteFile(generateStatesContent(), outputFiles.states),
      ].filter(Boolean)
    );
  } catch (error) {
...
export default generateColorRules;
