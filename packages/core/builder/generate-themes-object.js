...
const generateThemesObject = async (outputPath) => {
  ...
  const themeObjectsString = `export default ${JSON.stringify(
    themeObjects,
    null,
    null
  )}`;
  ...
  // types
  await generateThemesObjectDeclaration(
    outputPath.replace(".js", ".d.ts"),
    themeObjects
  );
};

export default generateThemesObject;
